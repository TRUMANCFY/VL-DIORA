...
class TreeLSTM(nn.Module):
    def __init__(self, size, ninput=2, leaf=False):
        ...
        # the leaf only need three parameters
        if leaf:
...
class DioraBase(nn.Module):
    ...
    def init_with_batch(self, h, c):
        ...
        # shape of h and c: [batch_size, length, size]
        # fill in the bottom values
    ...
    def forward(self, x):
        ...
        # h: (b, seq_len, emb_size); c: (b, seq_len, emb_size)
