...
def get_loss_funcs(options, batch_iterator=None, embedding_layer=None):
    ...
    elif options.reconstruct_mode == 'softmax' and options.emb != "bert":
        reconstruction_loss_fn = ReconstructionSoftmaxLoss(embedding_layer,
            margin=margin, k_neg=k_neg, input_size=input_dim, size=size, cuda=cuda)
    elif options.reconstruct_mode == 'softmax' and options.emb == 'bert':
        # (self, vocab_size, size, margin=1, k_neg=3, cuda=False)
...
class Trainer(object):
    ...
    def _step(self, batch_map, train=True, compute_loss=True, bert=False):
        if train:
            self.net.train()
        else:
            self.net.eval()
