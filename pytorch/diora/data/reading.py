...
class PartItTextReader(object):
    def __init__(self, lowercase=True, filter_length=0, delim=' '):
        self.lowercase = lowercase
        self.filter_length = filter_length if filter_length is not None else 0

    def read(self, file_dir):
        return self.read_sentences(file_dir)
    
    def read_sentences(self, file_dir):
        sentences = []

        extra = dict()
        # example_ids = []

        dir_list = [x for x in os.listdir(file_dir) if not x.startswith('.') and '.' not in x]
...
class PartItWholeTextReader(object):
    def __init__(self, lowercase=True, filter_length=0, delim=' '):
        self.lowercase = lowercase
        self.filter_length = filter_length if filter_length is not None else 0
        self.types = ['0.chair', '1.table', '2.bed', '3.bag']
    ...
    def read_sentences(self, file_dir):
        ...
        for type_ in self.types:
            ...
            file_dir_ = file_dir.format(type_)
            # print('formated file_dir: ', file_dir_)

            dir_list = [x for x in os.listdir(file_dir_) if '.' not in x]
            textfile_list = [
                os.path.join(file_dir_, dir_name, 'utterance.txt') for dir_name in dir_list
            ]
            tmp_sents = []
            for textfile in textfile_list:
                with open(textfile, 'r') as r:
                    # tmp = r.read().replace(".", "").replace(",", " , ").replace(":", " : ").replace(";", " ; ").replace("/", " ").replace("\'", " \'").replace("\"", " \"")
                    tmp = r.read().replace(",", " , ").replace(":", " : ").replace(";", " ; ").replace("/", " ").replace("\'", " \'").replace("\"", " \"")
                    # only remove the last dot, while for the dot in the middle, just replace them with space
                    tmp = re.sub('\.(?=$)', '', tmp)
                    tmp = tmp.replace('.', ' ')
                    tmp = tmp.strip().split()

                    if self.filter_length > 0 and len(tmp) > self.filter_length:
                        continue
                    
                    tmp_sents.append(tmp)
                    example_ids.append(textfile)
            
            print('The {} length: {}'.format(type_, len(tmp_sents)))
            sentences = sentences + tmp_sents
