...
class EmbeddingsReader(object):
    ...
    def context_insensitive_bert(self, *args, **kwargs):
        return context_insensitive_bert(*args, **kwargs)
    ...
    def get_emb_bert(self, options, word2idx):
        """
            Using bert embeddings
        """
        embeddings = self.context_insensitive_bert(word2idx=word2idx, cuda=options.cuda, cache_dir=None)
        return embeddings, word2idx
    ...
    def get_embeddings(self, options, embeddings_path, word2idx):
        if options.emb == 'w2v':
            ...
            out = self.get_emb_bert(options, word2idx)
        return out
...
def context_insensitive_bert(word2idx, cuda=False, cache_dir=None):
    logger = get_logger()
    ...
    if cache_dir is not None:
        key = hash_vocab(vocab)
        cache_path = os.path.join(cache_dir, 'bert_{}.npy'.format(key))

        if os.path.exists(cache_path):
            logger.info('Loading cached bert vecrtors: {}'.format(cache_path))
            return load_bert_cache(cache_path)
    ...
    logger.info('Begin caching vectors. nbatches={} device={}'.format(nbatches, device))
    logger.info('Initialize BERT Model.')
    ...
    for i in tqdm(range(nbatches), desc='bert'):
        start = i * batch_size
        batch = vocab[start:start+batch_size]
        if len(batch) == 0:
            continue
        ...
        for v_ in vec:
            print('v: ', v_)
            assert len(v_[0]) != 1 or len(v_[1]) != 1, 'The length does not match.'
            vec_.append(v_[1][0])
        vec_list.extend(vec_)
    ...
    if cache_dir is not None:
        logger.info('Saving cached bert vectors: {}'.format(cache_path))
        save_bert_cache(cache_path, vectors)
