...
from train_bert import argument_parser, parse_args, configure
from train_bert import get_validation_dataset, get_validation_iterator
from train_bert import build_net
...
def run(options):
    ...
    with torch.no_grad():
        for i, batch_map in tqdm(enumerate(batches)):
            token_masks = batch_map['sentences']['token_mask']
            sentences = batch_map['sentences']['origin_input_ids']
            batch_size = sentences.shape[0]
            length = token_masks.shape[1]
