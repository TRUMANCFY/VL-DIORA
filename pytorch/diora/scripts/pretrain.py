...
from diora.data.dataset import ConsolidateDatasets, ReconstructDataset, make_batch_iterator, Vocabulary, MLMDataset, BERTDataset
...
def get_train_dataset(options, tokenizer, model):
    ...
    return BERTDataset().initialize(options, tokenizer, model, text_path=options.train_path, filter_length=options.train_filter_length)
    # return MLMDataset().initialize(options, tokenizer, model, text_path=options.train_path, filter_length=options.train_filter_length)
...
def get_validation_dataset(options, tokenizer, model):
    return BERTDataset().initialize(options, tokenizer, model, text_path=options.validation_path, filter_length=options.validation_filter_length)
    # return MLMDataset().initialize(options, tokenizer, model, text_path=options.validation_path, filter_length=options.validation_filter_length)
...
def run(options):
    ...
    print(tokenizer.tokenize('this is a office chair that has a square arched back with two vertical arms connected to it and a thin bar that connects it to the square seat , it has a cylinder that connects the chair to the base and six curved legs in a star shape with wheels on each leg'))
    ...
    pbar = tqdm(zip(range(options.max_epoch), seeds))

    for epoch, seed in pbar:
        seed = seeds[epoch]
        for idx, batch in myiterator():
            optim.zero_grad()

            input_ids = batch['sentences']['input_ids'].to(device)
            attention_mask = batch['sentences']['attention_mask'].to(device)
            labels = batch['sentences']['labels'].to(device)
    ...
    tokenizer.save_pretrained(options.tokenizer_saving_path)
    model.save_pretrained(options.bertmodel_saving_path)
...
def argument_parser():
    ...
    # Add bert saving path
    parser.add_argument('--tokenizer_saving_path', default=None, type=str)
    parser.add_argument('--bertmodel_saving_path', default=None, type=str)
